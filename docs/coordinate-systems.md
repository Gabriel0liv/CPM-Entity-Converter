...
Isto não implica que Euler possa ser composto por soma em geral. Composição, reparenting e bake global devem usar matrizes/quaternions; Euler só é materializado no boundary do formato, com ordem de rotação confirmada por testes golden.
...
- rotation Gecko é delta em relação à rotação inicial: converter sinais/unidade uma vez e marcar `ADDITIVE`.
...
Aplicar a mesma regra por sample de animação. Qualquer singularidade de escala ou decomposição instável é erro. A opção preferida até o spike Q-002 é manter a árvore fonte intacta sob um anchor único, pois minimiza bake e drift.
...
- não normalizar cada frame isoladamente para `[-180,180]`, pois isso cria saltos;
