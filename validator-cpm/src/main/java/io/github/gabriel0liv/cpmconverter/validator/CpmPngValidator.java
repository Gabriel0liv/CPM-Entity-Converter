...
public final class CpmPngValidator {
  ...
  public Result<CpmPngMetadata> validate(byte[] bytes,CpmArtifactLimits limits){
    ...
    long pixels;
    long expected;
    try { pixels = Math.multiplyExact((long) width, (long) height); expected = Math.multiplyExact((long) height, Math.addExact(1L, Math.multiplyExact((long) width, 4L))); }
    catch (ArithmeticException ex) { return Result.failure(error(DiagnosticCodes.INPUT_LIMIT_EXCEEDED, "/IHDR", "PNG dimensions overflow")); }
    if (pixels > limits.maxPngPixels() || expected > limits.maxPngDecodedBytes() || expected > Integer.MAX_VALUE) return Result.failure(error(DiagnosticCodes.INPUT_LIMIT_EXCEEDED, "/IHDR", "decoded PNG budget exceeded"));
    ...
    catch (DataFormatException ex) { inflater.end(); return Result.failure(error(DiagnosticCodes.PNG_INVALID, "/IDAT", "invalid PNG zlib stream")); }
    boolean finished = inflater.finished();
    int remaining = inflater.getRemaining();
    inflater.end();
    if (offset != decoded.length || !finished || remaining != 0) return Result.failure(error(DiagnosticCodes.PNG_INVALID, "/IDAT", "PNG decoded stream length mismatch"));
