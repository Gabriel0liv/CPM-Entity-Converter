...
public final class CpmArtifactValidator {
  ...
  public Result<CpmValidatedArtifactV1> validate(CpmArtifactValidationRequest request) {
    ...
    int animations = 0;
    for (var entry : data.entries().entrySet()) if (entry.getKey().startsWith("animations/")) { var a = parser.parse(entry.getValue(), entry.getKey(), DiagnosticCodes.CPM_ANIMATION_INVALID); if (!a.success()) bag = bag.addAll(a.diagnostics()); else animations++; }
    ...
    if (bag.hasErrors()) return Result.failure(bag);
    return Result.success(new CpmValidatedArtifactV1(new CpmPersistedProjectV1(config), List.of(), data.inventory(), new CpmValidationSummary(statuses, canonical, roots, count, ids.size(), textured, animations, 0, 0, data.entries().containsKey("skin.png"), width, height)), bag);
  }
