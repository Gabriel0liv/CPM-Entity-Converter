...
public final class CpmArtifactValidator {
  ...
  public Result<CpmValidatedArtifactV1> validate(CpmArtifactValidationRequest request) {
    ...
    flat.clear(); generated.clear(); var orderedRoots = new ArrayList<CpmPersistedRootV1>();
    for (var root : rootList) { var orderedChildren = new ArrayList<CpmPersistedElementV1>(); for (int i=0;i<root.children().size();i++) orderedChildren.add(reindex(root.children().get(i), 0, "/elements/" + orderedRoots.size() + "/children/" + i, flat, generated)); orderedRoots.add(new CpmPersistedRootV1(root.id(), orderedChildren)); }
    rootList = orderedRoots;
    int sx=config.path("skinSize").path("x").asInt(data.entries().containsKey("skin.png")?64:1), sy=config.path("skinSize").path("y").asInt(data.entries().containsKey("skin.png")?64:1);
    var project = new CpmPersistedProjectV1(1, config.path("skinType").asText("default"), new CpmPersistedSize2i(Math.max(1,sx), Math.max(1,sy)), rootList, flat, generated, new CpmPersistedTextureV1("skin.png", width, height, config.path("textures").path("skin").path("customGridSize").asBoolean(false)));
  ...
  private CpmPersistedElementV1 reindex(CpmPersistedElementV1 e,int depth,String pointer,List<CpmPersistedElementV1> flat,Map<Long,CpmPersistedElementV1> generated){var children=new ArrayList<CpmPersistedElementV1>(); for(int i=0;i<e.children().size();i++) children.add(reindex(e.children().get(i),depth+1,pointer+"/children/"+i,flat,generated)); var n=new CpmPersistedElementV1(e.name(),e.storeId(),children,flat.size(),depth,pointer); flat.add(n); if(n.storeId()!=null) generated.put(n.storeId(),n); return n;}
