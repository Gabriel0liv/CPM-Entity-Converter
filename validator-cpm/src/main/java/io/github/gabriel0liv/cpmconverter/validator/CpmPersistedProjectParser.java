...
final class CpmPersistedProjectParser {
  ...
  CpmPersistedProjectV1 parse(JsonNode config,int width,int height,boolean texturePresent){
    ...
    return new CpmPersistedProjectV1(1,config.path("skinType").asText("default"),new CpmPersistedSize2i(sx,sy),roots,flat,ids,new CpmPersistedTextureV1("skin.png",width,height,config.path("textures").path("skin").path("customGridSize").asBoolean(false)));
  }
