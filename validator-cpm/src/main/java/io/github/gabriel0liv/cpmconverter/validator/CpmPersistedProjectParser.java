...
final class CpmPersistedProjectParser {
  ...
  CpmPersistedProjectV1 parse(JsonNode config, CpmPersistedSize2i validatedSize, int width,int height,boolean texturePresent){
    ...
    var targets=new LinkedHashMap<Long,CpmPersistedTargetV1>(); for(var root:roots){long id=switch(root.id()){case "head"->0;case "body"->1;case "left_arm"->2;case "right_arm"->3;case "left_leg"->4;case "right_leg"->5;default->-1;}; if(id>=0) targets.put(id,new CpmPersistedRootTargetV1(root));} for(var e:flat) if(e.storeId()!=null) targets.put(e.storeId(),new CpmPersistedElementTargetV1(e));
    return new CpmPersistedProjectV1(1,config.path("skinType").asText("default"),validatedSize,roots,flat,ids,targets,new CpmPersistedTextureV1("skin.png",width,height,config.path("textures").path("skin").path("customGridSize").asBoolean(false)));
  }
  ...
  private void collect(CpmPersistedElementV1 e,List<CpmPersistedElementV1> flat,Map<Long,CpmPersistedElementV1> ids){flat.add(e); if(e.storeId()!=null)ids.put(e.storeId(),e); for(var c:e.children())collect(c,flat,ids);}
}
