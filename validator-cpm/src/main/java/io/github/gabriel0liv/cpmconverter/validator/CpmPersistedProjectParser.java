...
final class CpmPersistedProjectParser {
  ...
  Result<CpmPersistedProjectV1> parse(JsonNode config, CpmValidatedConfigV1 validated, CpmPngMetadata png, boolean texturePresent, CpmArtifactLimits limits) {
    ...
    var roots = new ArrayList<CpmPersistedRootV1>(); var elements = new ArrayList<CpmPersistedElementV1>(); var generated = new LinkedHashMap<Long, CpmPersistedElementV1>(); var targets = new LinkedHashMap<Long, CpmPersistedTargetV1>(); var rootIds = new HashSet<String>(); int[] preorder = {0};
    for (int r = 0; r < rootsNode.size(); r++) {
      ...
      if (id == null || !ROOT_IDS.contains(id) || !rootIds.add(id)) { diagnostics.add(error(DiagnosticCodes.CPM_INVALID_ROOT, pointer + "/id", "unknown or duplicate root id")); continue; }
      ...
      CpmPersistedRootV1 rootValue = new CpmPersistedRootV1(id, rootId(id), optionalBoolean(root, "show", false, pointer + "/show", diagnostics), optionalBoolean(root, "showInEditor", true, pointer + "/showInEditor", diagnostics), optionalBoolean(root, "locked", false, pointer + "/locked", diagnostics), strictVec(root, "pos", pointer + "/pos", new CpmPersistedVec3(0, 0, 0), diagnostics), strictVec(root, "rotation", pointer + "/rotation", new CpmPersistedVec3(0, 0, 0), diagnostics), optionalBoolean(root, "dup", false, pointer + "/dup", diagnostics), optionalBoolean(root, "disableVanillaAnim", false, pointer + "/disableVanillaAnim", diagnostics), optionalText(root, "name", "", pointer + "/name", diagnostics), optionalInt(root, "nameColor", 0, pointer + "/nameColor", diagnostics), children, pointer);
      roots.add(rootValue);
