...
final class CpmPersistedProjectParser {
  ...
  CpmPersistedProjectV1 parse(JsonNode config,int width,int height,boolean texturePresent){
    ...
    int sx=config.path("skinSize").path("x").asInt(64), sy=config.path("skinSize").path("y").asInt(64);
    ...
    return new CpmPersistedProjectV1(1,config.path("skinType").asText("default"),new CpmPersistedSize2i(sx,sy),roots,flat,ids,targets,new CpmPersistedTextureV1("skin.png",width,height,config.path("textures").path("skin").path("customGridSize").asBoolean(false)));
  }
