...
final class CpmArtifactInspector {
  record InspectedEntry(String name, int method, LocalDateTime time, byte[] contents) {
  ...
  static InspectedArtifact inspect(byte[] bytes) throws IOException {
    ...
    try (var in = new ZipInputStream(new ByteArrayInputStream(bytes))) {
      ...
      while ((entry = in.getNextEntry()) != null) {
        ...
        entries.add(new InspectedEntry(entry.getName(), entry.getMethod(), entry.getTimeLocal(), out.toByteArray()));
      }
    }
