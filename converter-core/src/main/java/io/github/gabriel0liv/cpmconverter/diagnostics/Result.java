...
public final class Result<T> {
    ...
    private Result(T value, DiagnosticBag diagnostics) {
        if ((value == null) != diagnostics.hasErrors()) {
            throw new IllegalArgumentException("Result must be success(value, no errors) or failure(null, error)");
        }
        this.value = value;
        this.diagnostics = diagnostics;
    }

    public static <T> Result<T> success(T value) { return success(value, new DiagnosticBag()); }
    ...
    public DiagnosticBag diagnostics() { return diagnostics; }
    public boolean success() { return value != null && !diagnostics.hasErrors(); }
