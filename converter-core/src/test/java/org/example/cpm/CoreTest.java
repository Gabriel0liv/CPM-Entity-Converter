...
import org.junit.jupiter.api.*; import static org.junit.jupiter.api.Assertions.*; import org.example.cpm.math.*; import org.example.cpm.diagnostics.*; import java.util.*;
class CoreTest {
 ...
 @Test void diagnosticsDeterministic(){var b=new DiagnosticBag().add(Diagnostic.of(Severity.WARNING,"W","w")).add(Diagnostic.of(Severity.ERROR,"E","e"));assertTrue(b.hasErrors());assertEquals(List.of(Severity.WARNING,Severity.ERROR),b.all().stream().map(Diagnostic::severity).toList());assertThrows(IllegalArgumentException.class,()->new SourcePath("C:\\absolute.json"));}
